...
public class BaskingSharkEntity extends PathfinderMob implements GeoEntity {
	...
	public BaskingSharkEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(BenssharksModEntities.BASKING_SHARK.get(), world);
	}
		...
		this.moveControl = new MoveControl(this) {
			...
			public void tick() {
				if (BaskingSharkEntity.this.isInWater())
					BaskingSharkEntity.this.setDeltaMovement(BaskingSharkEntity.this.getDeltaMovement().add(0, 0.005, 0));
				if (this.operation == MoveControl.Operation.MOVE_TO && !BaskingSharkEntity.this.getNavigation().isDone()) {
					...
					BaskingSharkEntity.this.setYRot(this.rotlerp(BaskingSharkEntity.this.getYRot(), f, 10));
					...
					if (BaskingSharkEntity.this.isInWater()) {
						...
						BaskingSharkEntity.this.setXRot(this.rotlerp(BaskingSharkEntity.this.getXRot(), f2, 5));
					...
					BaskingSharkEntity.this.setSpeed(0);
					BaskingSharkEntity.this.setYya(0);
					BaskingSharkEntity.this.setZza(0);
				}
			}
		};
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "baskingshark");
	}
	...
	public InteractionResult mobInteract(Player sourceentity, InteractionHand hand) {
		...
		BaskingSharkRightClickedOnEntityProcedure.execute(world, x, y, z, entity, sourceentity);
		return retval;
	}
	...
	public EntityDimensions getDimensions(Pose p_33597_) {
		return super.getDimensions(p_33597_).scale((float) 1.9);
	}
	...
	public static void init() {
		SpawnPlacements.register(BenssharksModEntities.BASKING_SHARK.get(), SpawnPlacements.Type.IN_WATER, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(BaskingSharkEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
