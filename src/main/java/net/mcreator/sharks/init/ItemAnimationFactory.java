			...
			if (mainhandItem.getItem() instanceof SpetumItem animatable) {
				...
				if (!animation.isEmpty()) {
					...
					if (event.player.level().isClientSide()) {
						((SpetumItem) event.player.getMainHandItem().getItem()).animationprocedure = animation;
					}
				}
			}
			if (offhandItem.getItem() instanceof SpetumItem animatable) {
				animation = offhandItem.getOrCreateTag().getString("geckoAnim");
				if (!animation.isEmpty()) {
					event.player.getOffhandItem().getOrCreateTag().putString("geckoAnim", "");
					if (event.player.level().isClientSide()) {
						((SpetumItem) event.player.getOffhandItem().getItem()).animationprocedure = animation;
					}
				}
			}
		}
